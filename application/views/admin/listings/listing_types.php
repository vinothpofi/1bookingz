												...
												if ($row->name != 'accommodates' && $row->name != 'can_policy' && $row->name != 'minimum_stay' && $row->name != 'guest_capacity' && $row->name != 'SPACE_SIZE' && $row->name != 'No_of_beds' && $row->name != 'car_parking' && $row->name != 'No_of_bathrooms')
												{
													...
													if ($mode == '0')
													{
														...
														<a title="Click to inactive" class="tip_top"
														   href="javascript:confirm_status('admin/listings/change_listings_status/<?php echo $mode; ?>/<?php echo $row->id; ?>');"><span
											...
											if ($allPrev == '1' || in_array('2', $Listing)) { ?>
												<span>
													<a class="action-icons c-edit"
														 href="admin/listings/add_new_attribute/<?php echo $row->id; ?>"
														 title="Edit">Edit
													</a>
												</span>
											<?php 
												...
												<?php if ($allPrev == '1' || in_array('3', $Listing)) { ?>
													<span>
														<a class="action-icons c-delete"
															 href="javascript:confirm_delete('admin/listings/delete_list/<?php echo $row->id; ?>')"
															 title="Delete">Delete
														</a>
													</span>
												<?php }
