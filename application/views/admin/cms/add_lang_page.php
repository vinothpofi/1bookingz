			...
			<div class="widget_wrap">
				<div class="widget_wrap tabby">
					...
					<div class="widget_content">
						<?php
						$attributes = array('class' => 'form_container left_label', 'id' => 'adduser_form', 'accept-charset' => 'UTF-8', 'onsubmit' => 'return validate();');
						echo form_open('admin/cms/insert_lang_Cms', $attributes);
                        ...
                        $ownrattr = array(
                            ...
                            'Onchange'=>'get_main_pages(this.value)'
                        );
						?>
						<div id="tab1">
							<ul>
								...
								<li>
									<div class="form_grid_12">
										<?php
											...
											echo form_label('Choose Main Page <span class="req">*</span>','page_name', $commonclass);	
										?>
										<div class="form_input">
											<?php
												$minpg=array();
												$minpg[''] = 'Please Choose Main Page';

												/*foreach ($cms_details->result() as $page)
												{
													$minpg[$page->seourl]=$page->page_name;
												}	 
*/
												$minpgattr = array(
													    'id'       => 'main_page_div',
												...
												echo form_dropdown('seourl', $minpg, '', $minpgattr);
											?>
											<span id="page_name_valid" style="color:#f00;display:none;">Please Choose Main Page
											</span>
										</div>
									</div>
								</li>
								...
								<li>
									<div class="form_grid_12">
										...
										?>
										<div class="form_input">
											<select id="lang_warn" name="lang_code" tabindex="1" class="large tipTop"
													title="Choose the Language">
												<option value="">Please Choose Language</option>
											</select>
											<span id="lang_warn_valid" style="color:#f00;display:none;">Please select Language</span>
										</div>
									</div>
								</li>
								...
								<li>
									<div class="form_grid_12">
										...
										<div class="form_input">
											...
											?>
											<span
												id="page_name_valid" style="color:#f00;display:none;">Only Characters allowed!
											</span>
											<span id="page_name_text_valid" style="color:#f00;display:none;">Page Name is Required
											</span>
										</div>
									</div>
								</li>
								...
								<li>
									<div class="form_grid_12">
										...
										<div class="form_input">
											...
											?>
											<span
												id="page_title_valid" style="color:#f00;display:none;">Page Title is Required
											</span>
										</div>
									</div>
								</li>
...
<script>
	function validate() 
	{
		if ($('#page_name option:selected').val() == '') 
		{
		...
		if ($('#lang_warn option:selected').val() == '')
		{
			document.getElementById("lang_warn_valid").style.display = "inline";
			$("#lang_warn").focus();
			$("#lang_warn_valid").fadeOut(5000);
			return false;
		}

		if ($('#page_name_text').val() == '')
		{
			document.getElementById("page_name_text_valid").style.display = "inline";
			$("#page_name_text").focus();
			$("#page_name_text_valid").fadeOut(5000);
			return false;
		}
	...
	function  get_main_pages(top_menu_id) {

	    if(top_menu_id){
            $.ajax(
                {
                    type: 'POST',
                    url: '<?php echo base_url(); ?>admin/cms/get_main_page',
                    data: {'top_menu_id': top_menu_id},
                    success: function (data)
                    {
                        $('#main_page_div').html(data);
                    }
                });
        }
    }
    function main_page(val1)
    {
        $.ajax(
            {
                type: 'POST',
                url: '<?php echo base_url(); ?>admin/cms/main_page',
                data: {'seourl': val1},
                success: function (data)
                {
                    $('#lang_warn').html(data);
