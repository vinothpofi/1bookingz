...
?>
	<script type="text/javascript">
		function checkHostPaypalEmail(id) 
		{
			...
			{
				alert('No Guest Paypal Email Given');
				return false;
			}
		}
	</script>
										...
										<tr>
											...
											<td>
											 ...
											 //    {
												...
												// 		//convertCurrency($cancel->user_currencycode, $admin_currency_code, $cancel_amountWithSecDeposit,$currencyCronId);
												...
												// 	echo $admin_currency_symbol . ' ' . $$cancel->totalAmt;
												// }
                                                //
                                                //
                                                //echo $admin_currency_symbol . ' ' . $cancel->totalAmt;
                                                ...
                                                if ($admin_currency_code != $cancel->currency_code)
                                                {
                                                    echo $admin_currency_symbol . ' ' .currency_conversion($cancel->currency_code, $admin_currency_code, $cancel->totalAmt,$currencyCronId);
                                                    echo "--" . $cancel->booking_no;
                                                }
                                                else
                                                {
                                                    echo $admin_currency_symbol . ' ' . number_format($cancel->totalAmt,2);
                                                    echo "--" . $cancel->booking_no;
                                                }
												...
												if ($cancel->paid_canel_status == 1)
												{
													...
													{  /*for moderate,flexible*/
														if ($admin_currency_code != $cancel->user_currencycode) 
														{
															...
															//convertCurrency($cancel->user_currencycode, $admin_currency_code, $cancel->paid_cancel_amount,$currencyCronId);//customised_currency_conversion($currencyPerUnitSeller, $cancel->paid_cancel_amount);
														} 
														else
														{
...
*/
											...
											<td>
												...
												if ($cancel->paid_canel_status == 0)
												{
													if ($cancel->cancel_percentage != '100')
													{ /* for moderate,flexible*/
														$sec_dep = $cancel->payable_amount - $cancel->subTotal;
								                        $can_per = $cancel->subTotal/100 * $cancel->cancel_percentage;
								                        $pay_can =$cancel->subTotal - $can_per;
								                        $cancel_amt_is = $sec_dep + $pay_can;
														if ($admin_currency_code != $cancel->user_currencycode) 
														{
															$Balence = changeCurrency($cancel->user_currencycode,$admin_currency_code,$cancel_amt_is,$currencyCronId);
											...
											<td>
												...
												if ($cancel->paid_canel_status == 0)
												{
													...
													{ /*for moderate,flexible*/
														...
														if ($admin_currency_code != $cancel->user_currencycode)
														{
															$cancel_amount_pay =  changeCurrency($cancel->user_currencycode,$admin_currency_code,$cancel_amt_is,$currencyCronId);
															//convertCurrency($cancel->user_currencycode, $admin_currency_code, $cancel->paid_cancel_amount,$currencyCronId);
															//customised_currency_conversion($currencyPerUnitSeller, $cancel->paid_cancel_amount);
														}
														else
														{
															$cancel_amount_pay = $cancel_amt_is;
														}
	...
	<span class="action_link">
														<a class="p_approve tipLeft" href="admin/dispute/add_pay_form/<?php echo $cancel->booking_no; ?>/<?php echo $cancel->id; ?>" title="Offline Payment Mode" style="margin-bottom:2px;">Paid</a>
													</span>
												<?php } ?>
											</td>
										</tr>
