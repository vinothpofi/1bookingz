			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						?>
						<tr>
							<td class="center tr_select <?php echo $currencyCronId;?>">
								<?php
									echo form_input([
										'type'     => 'checkbox',
										'value'    => $row->id,
							...
							<td class="center">
								<?php //echo $row->totalAmt;								if($row->user_currencycode!=$row->currencycode)								{									/*$subTotal = ceil(currency_conversion($row->user_currencycode, $row->currencycode, $row->subTotal,$currencyCronId));									$serviceFee = ceil(currency_conversion($row->user_currencycode, $row->currencycode, $row->serviceFee,$currencyCronId));									$securityDep = ceil(currency_conversion($row->user_currencycode, $row->currencycode, $row->secDeposit,$currencyCronId));									echo number_format(($subTotal+$serviceFee+$securityDep),2);*/									echo currency_conversion($row->user_currencycode, $row->currencycode, $row->totalAmt,$currencyCronId);																	}								else								{									echo $row->totalAmt;								}																								?>			
							</td>
							<td class="center">
								<?php 
