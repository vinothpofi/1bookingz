...
foreach($getCustomerDetails as $myrow) {
	...
	$currencyPerUnitSeller=$myrow[currencyPerUnitSeller];
		...
		{			
			$admin_price = $admin_currency_symbol.' '.currency_conversion($myrow[user_currencycode], $admin_currency_code, $myrow[totalAmt],$currencyCronId);;//.customised_currency_conversion($currencyPerUnitSeller,$myrow[totalAmt]);		
		}else
	...
	$XML .='<tr>
		...
		if($status=='Completed')
		{			if($myrow[user_currencycode]!=$myrow[currencycode])			{				$hostAmt =  number_format(ceil(currency_conversion($myrow[user_currencycode], $myrow[currencycode], $myrow[totalAmt],$currencyCronId)),2);			}			else			{				$hostAmt =  $myrow[totalAmt];			}
			$XML .='<td style="text-align:left">'.ucfirst($myrow[currencycode]).'</td>
					<td style="text-align:left">'.ucfirst($hostAmt).'</td>
