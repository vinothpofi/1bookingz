<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Review_model extends My_Model
    {
        ...
        {
            $this->db->select('d.*, p.id as product_id,p.seourl, p.product_title,u.email');
        ...
        {
            $this->db->select('d.*,d.email as d_email, p.id as product_id, p.seourl,p.product_title,u.email');
            $this->db->from(DISPUTE . ' as d');
            $this->db->join(PRODUCT . ' as p', "d.prd_id=p.id", "LEFT");
            $this->db->join(USERS . ' as u', "u.id = d.user_id", "LEFT");
            $this->db->order_by('d.created_date', 'desc');
        ...
        {
            $this->db->select('d.*,p.product_title,p.seourl,u.user_name,u.email,mm.senderId as med_senderid,mm.receiverId as med_receiverid,p.user_id as host_id');
            $this->db->from(DISPUTE . ' as d');
            $this->db->join(PRODUCT . ' as p', "d.prd_id=p.id", "LEFT");
        ...
        {
            $select_qry = "SELECT d.*,p.product_title,p.seourl,u.image,host.firstname,host.lastname from " . DISPUTE . " d 
        ...
        function get_cancel_dispute_new($user_id = '', $pageLimitStart = '', $searchPerPage = '')
        {
            ...
            $this->db->select('d.*,p.product_title,p.seourl,p.user_id,u.image,u.firstname,u.lastname',false);
            $this->db->from(DISPUTE . ' as d');
