...
class Order_model extends My_Model
{
	...
	public function view_order_details($status,$condition='',$rep_code=''){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_name,pd.id as PrdID,rq.Bookingno as bookingno,rq.currencyPerUnitSeller,rq.currency_cron_id');
		$this->db->from(PAYMENT.' as p');
		$this->db->join(USERS.' as u' , 'p.user_id = u.id');
		$this->db->join(PRODUCT.' as pd' , 'pd.id = p.product_id');	
         $this->db->join(RENTALENQUIRY.' as rq' , 'rq.id = p.EnquiryId');
		 if($rep_code!=''){
			 $this->db->join(USERS.' as h' , 'h.id=rq.renter_id');
		 }
		$this->db->where('p.status = "'.$status.'"'.$condition);	
	...
	public function view_order_cod($status){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_name,pd.id as PrdID');
		$this->db->from(PAYMENT.' as p');
		$this->db->join(USERS.' as u' , 'p.user_id = u.id');
		$this->db->join(PRODUCT.' as pd' , 'pd.id = p.product_id');		
	...
	public function view_order_detailsexcel($status='',$condition='',$rep_code=''){
	...
	public function view_host_detailsexcel($condition){
	...
	$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_name,pd.id as PrdID,pd.product_title as prd_name');
		...
		$this->db->join(PRODUCT.' as pd' , 'pd.id = p.product_id');		
		$this->db->where('p.payment_status = "paid"'.$condition);
		$this->db->order_by("p.created", "desc"); 
		$PrdList = $this->db->get();
...
/*********************************************** Payment Success Cart********************************************************/	
	...
	public function PaymentSuccess($userid='', $randomId='' ,$transId = '', $payerMail = ''){
		...
		$user_booked = $this->order_model->get_all_details(PAYMENT,array('user_id'=>$userid,'status'=>'Paid'));
		//echo $user_booked->num_rows();
		 // $curren_enq_id = $this->db->select('currency_cron_id')->where('id',$_SESSION['EnquiryId'])->get('fc_rentalsenquiry')->row()->currency_cron_id;
			 
			//  echo $curren_enq_id;exit();
		if($user_booked->num_rows()==0)
		{
			$booked_data_Q = $this->order_model->get_all_details(PAYMENT,array('user_id'=>$userid,'dealCodeNumber'=>$randomId));
			...
			$book_commission_query = 'SELECT * FROM ' . INVITE . ' WHERE email = "'.$referred_user->row()->email.'"';
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_name,pd.image,pd.id as PrdID,pAr.attr_name');
		$this->db->from(PAYMENT.' as p');
		$this->db->join(USERS.' as u' , 'p.user_id = u.id');
		$this->db->join(PRODUCT.' as pd' , 'pd.id = p.product_id');		
...
$message.='</td>
   ...
   /* COnfirm booking for COD end */
	...
	public function view_listingorder_details($status,$condition=''){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_name,pd.id as PrdID,pd.product_title as prd_name');
		$this->db->from(HOSTPAYMENT.' as p');
		$this->db->join(USERS.' as u' , 'p.host_id = u.id');
		$this->db->join(PRODUCT.' as pd' , 'pd.id = p.product_id');		
		$this->db->where('p.payment_status = "'.$status.'"'.$condition);	
		$this->db->order_by("p.created", "desc"); 
