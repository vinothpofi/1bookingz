...
class Couponcards_model extends My_Model
{
	...
	public function view_product_details($ArrVal = '')
	{
		...
		if ($ArrVal != '') {
			foreach ($productList->result() as $PrdRow) {
				if (in_array($PrdRow->id, $ArrVal) == 1) {
					$condT = ' checked="checked"';
				} else {
					$condT = '';
				}
	...
	public function paymentcounponcount($couponcode = '')
	{
		$this->db->select('count(*) as cntval');
		$this->db->from(PAYMENT);
		$this->db->where(PAYMENT . '.couponCode', $couponcode);
		$query = $this->db->get();
