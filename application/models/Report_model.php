...
public function get_all_cancelled_users()
	...
	public function get_allthe_details_users($status, $search_name, $checkin, $checkout, $id, $rep_code = '', $limit_per_page, $start_index)
	{
		$whereCond = '';
		if ($status != '') {
			if($status == 'UnPublish'){
...
$status = 'Active';
			...
			$whereCond = ' u.status="' . $status . '" and';
		}
		$group = 'User';
		$whereCond .= ' u.group="' . $group . '" ';
		if ($search_name != '') {
			if ($whereCond != '') {
				$whereCond .= ' and ';
			} 
			$whereCond .= ' u.firstname LIKE "%' . $search_name . '%" ';
		}
		if ($checkin != '' && $checkout != '') {
			if ($whereCond != '') {
				$whereCond .= ' and ';
			} 
			$whereCond .= 'pr.dateAdded >="' . $checkin . '" AND pr.dateAdded<="' . $checkout . '" ';
			
		} 
		
	//echo $whereCond;exit();
		 $this->db->select('u.*,pr.*,count(case when pr.cancelled = "Yes" then pr.cancelled end) as cacelled_pro_count,count(case when pr.booking_status = "Booked" and pr.cancelled = "No" then pr.booking_status end) as booked_pro_count,count(case when pr.booking_status = "Pending" or pr.booking_status = "Enquiry" then pr.booking_status end) as pending_pro_count,count(case when pr.prd_id then pr.prd_id end) as total_order');
           ...
           $this->db->limit($limit_per_page,$start_index);
            return $productList = $this->db->get_where();

  //           $groupby = 'group by u.id';
		// $select_qry = "u.*,pr.*,er.*,count(case when pr.booking_status = Booked then pr.booking_status end) as booked_pro_count,count(case when pr.booking_status = Pending or pr.booking_status = Enquiry then pr.booking_status end) as pending_pro_count,count(case when pr.prd_id then pr.prd_id end) as total_order,count(case when er.booking_status = Booked then er.booking_status end) as booked_exp_count,count(case when er.booking_status = Pending or er.booking_status = Enquiry then pr.booking_status end) as pending_exp_count,count(case when er.prd_id then er.prd_id end) as exp_total_order
		// from " . USERS . " u 
		// LEFT JOIN " . RENTALENQUIRY . " pr on pr.user_id=u.id
		// LEFT JOIN " . EXPERIENCE_ENQUIRY . " er on er.user_id=u.id
	...
	public function get_exp_det($email = '')
	{
		$user_id_is = $this->db->where('email', $email)->get(USERS)->row()->id;
 $this->db->select('er.*,count(case when er.prd_id then er.prd_id end) as exp_total_order,count(case when er.cancelled = "Yes" then er.cancelled end) as cacelled_exp_count,count(case when er.booking_status = "Booked" and er.cancelled = "No" then er.booking_status end) as booked_exp_count,count(case when er.booking_status = "Pending" or er.booking_status = "Enquiry" then er.booking_status end) as pending_exp_count');
 $this->db->from(EXPERIENCE_ENQUIRY . ' as er');
 $this->db->where(array('er.renter_id' => $user_id_is));
  $exp_reportList = $this->db->get_where();
   return $exp_reportList =   $exp_reportList->row();

	}
	public function get_allthe_details_hosts($status, $search_name, $checkin, $checkout, $id, $rep_code = '', $limit_per_page, $start_index)
	{
		//$whereCond = '';
		if ($status != '') {
			if($status == 'UnPublish'){
...
$status = 'Active';
		...
		if ($search_name != '') {
			...
			$whereCond .= ' u.firstname LIKE "%' . $search_name . '%" and';
		}
		if ($checkin != '' && $checkout != '') {
			// if ($whereCond != '') {
			// 	$whereCond .= ' and ';
			// } 
			$whereCond .= ' pr.dateAdded >="' . $checkin . '" AND pr.dateAdded<="' . $checkout . '" and ';
		} 
		if($whereCond == '')
		{$Seller = 'Seller';
				$whereCond = ' u.group="' . $Seller . '" ';}
				else{
					$Seller = 'Seller';
				$whereCond .= ' u.group="' . $Seller . '" ';
				}
		...
		//echo $whereCond;exit();
				 // $this->db->select('u.*,pr.*,er.*,count(case when pr.cancelled = "Yes" then pr.cancelled end) as cacelled_pro_count,count(case when pr.booking_status = "Booked" then pr.booking_status end) as booked_pro_count,count(case when pr.booking_status = "Pending" or pr.booking_status = "Enquiry" then pr.booking_status end) as pending_pro_count,count(case when pr.prd_id then pr.prd_id end) as total_order,count(case when er.prd_id then er.prd_id end) as exp_total_order,count(case when er.cancelled = "Yes" then er.cancelled end) as cacelled_exp_count,count(case when er.booking_status = "Booked" then er.booking_status end) as booked_exp_count,count(case when er.booking_status = "Pending" or er.booking_status = "Enquiry" then er.booking_status end) as pending_exp_count');
     //        $this->db->from(USERS . ' as u');
	...
	public function get_allthe_details_experience($status, $search_name, $checkin, $checkout, $id, $rep_code = '', $limit_per_page, $start_index)
	{
		$whereCond = '';
		if ($status != '') {
			if($status == 'UnPublish'){
...
$status = 1;
		...
		if ($search_name != '') {
			...
			$whereCond .= ' p.experience_title LIKE "%' . $search_name . '%" ';
		}
		if ($checkin != '' && $checkout != '') {
			if ($whereCond != '') {
				$whereCond .= ' and ';
			...
			$whereCond .= 're.dateAdded >="' . $checkin . '" AND re.dateAdded<="' . $checkout . '"';
		} elseif ($checkin != '') {
			if ($whereCond != '') {
				$whereCond .= ' and ';
			} else {
			...
			$whereCond .= ' p.dateAdded <= "' . $checkout . '" ';
		}
		if ($id != '') {
			if ($whereCond != '') {
				$whereCond .= ' and ';
		...
		$select_qry = "select p.*,pa.city as city_name,et.id as et_id,et.experience_title as cat_title,u.firstname,u.lastname,u.image as user_image,u.feature_product,u.phone_no,u.email,u.address,u.address2,u.city as addr3,pa.lat as latitude,pa.lang as longitude,ph.product_image as PImg,p.featured,count(case when re.booking_status = 'Booked' and re.cancelled = 'No' then re.booking_status end) as booked_pro_count,count(case when re.booking_status = 'Pending' or re.booking_status = 'Enquiry' then re.booking_status end) as pending_pro_count,count(case when re.cancelled = 'Yes' then re.cancelled end) as cacelled_pro_count,count(case when re.prd_id then re.prd_id end) as total_order
		from " . EXPERIENCE . " p 
		...
		LEFT JOIN " . EXPERIENCE_ENQUIRY . " re on (re.prd_id=p.experience_id)
		" . $whereCond . " 
		" . $groupby . "  
	...
	public function get_allthe_details($status, $search_name, $checkin, $checkout, $id, $rep_code = '', $limit_per_page, $start_index)
	{
		$whereCond = '';
		if ($status != '') {
		...
		if ($search_name != '') {
			...
			$whereCond .= ' p.product_title LIKE "%' . $search_name . '%" ';
		}
		if ($checkin != '' && $checkout != '') {
			if ($whereCond != '') {
				$whereCond .= ' and ';
		...
		$select_qry = "select p.*,pa.city as city_name,u.firstname,u.lastname,u.image as user_image,u.feature_product,u.phone_no,u.email,u.address,u.address2,u.city as addr3,pa.lat as latitude,pa.lang as longitude,pb.datefrom as book_datefrom,pb.dateto as book_dateto,ph.product_image as PImg,p.featured,cur.currency_symbols,count(case when re.booking_status = 'Booked' and re.cancelled = 'No' then re.booking_status end) as booked_pro_count,count(case when re.cancelled = 'Yes' then re.cancelled end) as cacelled_pro_count,count(case when re.booking_status = 'Pending' or re.booking_status = 'Enquiry' then re.booking_status end) as pending_pro_count,count(case when re.prd_id then re.prd_id end) as total_order from " . PRODUCT . " p 
		...
		LEFT JOIN " . USERS . " u on (u.id=p.user_id)
		LEFT JOIN " . RENTALENQUIRY . " re on (re.prd_id=p.id)
	...
	public function get_search_options_experience($condition)
	{
	...
	public function view_product_details_site($condition = '')
	{
        ...
        if(count($neighbor_overviewlangarray)>0) {
			...
			$cancel_descriptionlangarray=language_dynamic_admin_enable_submit(array('p.neighbor_overview'),2); //Need to Update here
		...
		$select_qry = "select p.product_name,p.product_title,p.product_title_ar,p.id,p.seourl,p.price,p.price_range,p.description,p.description_ar,p.user_id,p.list_name,p.list_value,p.comment_count,p.status,p.home_type,p.room_type,p.currency,p.instant_pay,p.request_to_book,p.space".$spacelangarray.",p.other_thingnote".$other_thingnotelangarray.",p.house_rules".$house_ruleslangarray.",p.guest_access".$guest_accesslangarray.",p.interact_guest".$interact_guestlangarray.",p.listings,p.cancellation_policy,p.security_deposit,p.cancel_description,p.cancel_description_ar,p.neighbor_overview".$neighbor_overviewlangarray.",p.meta_title".$meta_titlelangarray.",p.meta_keyword".$meta_keywordlangarray.",p.meta_description".$meta_descriptionlangarray.",p.order,p.contact_count,
						u.id as userid,
	...
	{
		$select_qry = "select p.id,p.price,p.seourl,p.product_title,p.currency,
						...
						pa.state as statename,lsv.list_value,(select IFNULL(count(R.id),0) from " . REVIEW . " as R where R.product_id= p.id and R.status='Active') as num_reviewers , (select IFNULL(avg(Rw.total_review),0) from " . REVIEW . " as Rw where Rw.product_id= p.id and Rw.status='Active') as avg_val from " . PRODUCT . " p 
		LEFT JOIN " . PRODUCT_ADDRESS_NEW . " pa on pa.productId=p.id
	...
	function view_product_details_booking($condition = '')
	{
		$select_qry = "select p.*,
						...
						pa.latitude,pa.longitude,pa.address,pa.post_code,rq.currencycode,
						c.name as city_name,(select AVG(c.total_review) FROM fc_review c where c.product_id=p.id and c.status='Active') as rate, (select count(c.id) FROM fc_review c where c.product_id=p.id and c.status='Active') as num_reviewers,
	...
	{
		return $this->db->select(REVIEW . '.id')->join(PRODUCT, PRODUCT . '.id=' . REVIEW . '.product_id')->where(REVIEW.'.status','Active')->where(PRODUCT . '.user_id', $host_id)->get(REVIEW)->num_rows();
	}
	...
	{
		return $this->db->select(EXPERIENCE_REVIEW . '.id')->join(EXPERIENCE, EXPERIENCE . '.experience_id=' . EXPERIENCE_REVIEW . '.product_id')->where(EXPERIENCE . '.user_id', $host_id)->where(EXPERIENCE_REVIEW.'.status','Active')->get(EXPERIENCE_REVIEW)->num_rows();
	}
	...
	{
		$select_qry = "SELECT p.*,pa.city,lsv.list_value,lsv.list_value_ar,u.image as user_image,AVG(r.total_review ) as rate,rp.product_image as product_image FROM " . PRODUCT . " p 
			JOIN " . USERS . " u on u.id=p.user_id
			LEFT JOIN " . REVIEW . " r on r.product_id=p.id 
			LEFT JOIN " . PRODUCT_ADDRESS_NEW . " pa on pa.productId=p.id 
	...
	{
		$this->db->select('p.*,lsv.list_value,pa.city,u.image as user_image,  (select AVG(c.total_review) FROM fc_review c where c.product_id=p.id and c.status="Active") as rate, (select count(c.id) FROM fc_review c where c.product_id=p.id and c.status="Active") as num_reviewers,rp.product_image as product_image');
		$this->db->from(PRODUCT . ' as p');
		$this->db->join(USERS . ' as u', "u.id=p.user_id");
	...
	public
	function get_all_properties($start = "", $perPage = "")
	{
		$this->db->select('p.*,lsv.list_value,lsv.list_value_ar,pa.city,u.image as user_image,  (select AVG(c.total_review) FROM fc_review c where c.product_id=p.id and c.status="Active") as rate, (select count(c.id) FROM fc_review c where c.product_id=p.id and c.status="Active") as num_reviewers,rp.product_image as product_image');
		$this->db->from(PRODUCT . ' as p');
		$this->db->join(USERS . ' as u', "u.id=p.user_id");
		$this->db->join(REVIEW . ' as r', "r.product_id=p.id", "LEFT");
		$this->db->join(PRODUCT_PHOTOS . ' as rp', "rp.product_id=p.id", "LEFT");
