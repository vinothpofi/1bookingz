 ...
 *
 */
class Experience_order_model extends My_Model
{

	public function add_order($dataArr = '')
	{
		$this->db->insert(EXPERIENCE, $dataArr);
	}

	public function edit_order($dataArr = '', $condition = '')
	{
		$this->db->where($condition);
		$this->db->update(EXPERIENCE, $dataArr);
	}

	public function view_order($condition = '')
	{
		return $this->db->get_where(EXPERIENCE, $condition);
	}

	public function view_order_details($status)
	{
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.currency,pd.experience_id as PrdID,rq.Bookingno as bookingno,rq.currencycode,rq.currencyPerUnitSeller,rq.currency_cron_id');
		...
		$this->db->join(EXPERIENCE_ENQUIRY . ' as rq', 'rq.id = p.EnquiryId');
	...
	public function view_order_cod($status)
	{
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.experience_id as PrdID');
		$this->db->from(EXPERIENCE_BOOKING_PAYMENT . ' as p');
		$this->db->join(USERS . ' as u', 'p.user_id = u.id');
		$this->db->join(EXPERIENCE . ' as pd', 'pd.experience_id = p.product_id');
		$this->db->where('p.status = "' . $status . '"');
	...
	public function view_order_detailsexcel($status = '')
	{
	
		
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.currency,pd.experience_id as PrdID,rq.currencycode,rq.Bookingno as bookingno,rq.currencyPerUnitSeller,rq.currency_cron_id');
		$this->db->from(EXPERIENCE_BOOKING_PAYMENT . ' as p');
		$this->db->join(USERS . ' as u', 'p.user_id = u.id');
		$this->db->join(EXPERIENCE . ' as pd', 'pd.experience_id = p.product_id');
		$this->db->join(EXPERIENCE_ENQUIRY . ' as rq', 'rq.id = p.EnquiryId');
	...
	public function view_host_detailsexcel()
	{
	
		
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,pd.price,u.country,u.city,pd.experience_title as product_name,pd.experience_id as PrdID,pd.experience_title as prd_name,pd.currency as hosted_currency');
	...
	public function PaymentSuccess($userid = '', $randomId = '', $transId = '', $update_pay_type = '')
	{  //$payerMail
		$paymtdata = array(
			'randomNo' => $randomId,
		...
		/* $CoupRes = $this->experience_order_model->get_all_details(SHOPPING_CART,array( 'user_id' => $userid, 'couponID >' => 0));
		...
		if($couponID != 0) {
			if($couponType == 'Gift'){
				$SelGift = $this->experience_order_model->get_all_details(GIFTCARDS,array( 'id' => $couponID));
				$GiftCountValue = $SelGift->row()->used_amount + $couponAmont;
				$condition = array( 'id' => $couponID);
				$dataArr = array('used_amount' => $GiftCountValue);
				$this->experience_order_model->update_details(GIFTCARDS,$dataArr,$condition);
				if($SelGift->row()->price_value <= $GiftCountValue ){
					
					$condition1 = array( 'id' => $couponID);
					$dataArr1 = array('card_status' => 'redeemed');
					$this->experience_order_model->update_details(GIFTCARDS,$dataArr1,$condition1);
				}
			
			}else{
				$SelCoup = $this->experience_order_model->get_all_details(COUPONCARDS,array( 'id' => $couponID));
				$CountValue = $SelCoup->row()->purchase_count + 1;
				$condition = array( 'id' => $couponID);
				$dataArr = array('purchase_count' => $CountValue);
				$this->experience_order_model->update_details(COUPONCARDS,$dataArr,$condition);
				//echo '<br>'.$this->db->last_query();
			}
		} */
		//die;
		...
		$this->experience_order_model->update_details(EXPERIENCE_BOOKING_PAYMENT, $dataArr1, $condition1);
		//Update Quantity
		/* $SelQty = $this->experience_order_model->get_all_details(PAYMENT,array( 'user_id' => $userid, 'dealCodeNumber' => $randomId));
        ...
        foreach($SelQty->result() as $updPrdRow){
            ...
            $SelPrd = $this->experience_order_model->get_all_details(PRODUCT,array( 'id' => $updPrdRow->product_id ));
            ...
            $this->experience_order_model->update_details(PRODUCT,$dataArr2,$condition2);
        } */
	...
	public function PaymentGiftSuccess($userid = '', $transId = '', $payerMail = '')
	{
		...
		$this->session->set_userdata($paymtdata);
		$GiftTemp = $this->experience_order_model->get_all_details(GIFTCARDS_TEMP, array('user_id' => $userid));
		foreach ($GiftTemp->result() as $GiftRows) {
			...
			$condition = '';
			$this->experience_order_model->simple_insert(GIFTCARDS, $dataArr);
		}
		...
		$this->experience_order_model->update_details(GIFTCARDS, $dataArr1, $condition1);
		//Send Mail to User
		$GiftTempVal = $this->experience_order_model->get_all_details(GIFTCARDS, array('user_id' => $userid));
		$this->SendMailUSersGift($GiftTempVal);
		//Empty Gift cart Temp Info
		$condition3 = array('user_id' => $userid);
		$this->experience_order_model->commonDelete(GIFTCARDS_TEMP, $condition3);
		echo 'Success';
	}
	...
	public function PaymentSubscribeSuccess($userid = '', $transId = '')
	{
		...
		$this->session->set_userdata($paymtdata);
		$FancyboxTemp = $this->experience_order_model->get_all_details(FANCYYBOX_TEMP, array('user_id' => $userid));
		foreach ($FancyboxTemp->result() as $FancyboxRow) {
			...
			$condition = '';
			$this->experience_order_model->simple_insert(FANCYYBOX_USES, $dataArr);
		}
		$condition1 = array('user_id' => $userid);
		$dataArr1 = array('status' => 'Paid', 'trans_id' => $transId, 'payment_type' => 'Credit Cart');
		$this->experience_order_model->update_details(FANCYYBOX_USES, $dataArr1, $condition1);
		//Update Quantity
		foreach ($FancyboxTemp->result() as $updPrdRow) {
			$SelPrd = $this->experience_order_model->get_all_details(FANCYYBOX, array('id' => $updPrdRow->fancybox_id));
			...
			$this->experience_order_model->update_details(FANCYYBOX, $dataArr2, $condition2);
		}
		//Send Mail to User
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city');
		...
		//Empty Gift cart Temp Info
		$condition3 = array('user_id' => $userid);
		$this->experience_order_model->commonDelete(FANCYYBOX_TEMP, $condition3);
		echo 'Success';
	}
	...
	public function SendMailUSers($PrdList, $SellList)
	{
		...
		$shipAddRess = $this->experience_order_model->get_all_details(SHIPPING_ADDRESS, array('id' => $PrdList->row()->shippingid));
...
<div style="float:left; width:100%; margin-right:3%; margin-top:10px; font-size:14px; font-weight:normal; line-height:28px;  font-family:Arial, Helvetica, sans-serif; color:#000; overflow:hidden;">   
	...
	public function SendMailUSersGift($GiftRowsVals)
	{
		...
		foreach ($GiftRowsVals->result() as $GiftVals) {
			...
			$usrDetails = $this->experience_order_model->get_all_details(USERS, array('id' => $GiftVals->user_id));
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	{
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.image,pd.experience_id as PrdID');
		$this->db->from(EXPERIENCE_BOOKING_PAYMENT . ' as p');
		$this->db->join(USERS . ' as u', 'p.user_id = u.id');
		$this->db->join(EXPERIENCE . ' as pd', 'pd.experience_id = p.product_id');
...
<div style="float:left; width:100%; margin-right:3%; margin-top:10px; font-size:14px; font-weight:normal; line-height:28px;  font-family:Arial, Helvetica, sans-serif; color:#000; overflow:hidden;">   
	...
	/* Confirm booking for COD */
	public function getbooking_detail($status = '')
	{
		$this->db->select('*');
		$this->db->from(EXPERIENCE_BOOKING_PAYMENT);
		if ($status != '') {
			$this->db->where(EXPERIENCE_BOOKING_PAYMENT . '.payment_type', $status);
		}
		$query = $this->db->get();
		$resultContent = $query->result_array();
		return $resultContent;
	}
	/* COnfirm booking for COD end */
	/* Host payment listing */
	public function view_listingorder_details($status)
	{
