...
class Signupsignin extends MY_Controller
{
    ...
    function create_normal_user($message)
    {
        ...
        if (valid_email($email)) {
            ...
            if ($duplicateMail->num_rows() > 0) {
				...
				$login_type='normal';
				$rep_code='';
				$this->user_model->insertUserQuick($firstname, $lastname, $email, $pwd, $confirm_password, $expireddate, $login_type, $this->input->post('birth_day'), $this->input->post('birth_month'), $this->input->post('birth_year'), $invite_reference, $rep_code);
    ...
    function login_normal_user()
    {
        ...
        $expireddate = date('Y-m-d', strtotime('+15 days'));
        if (valid_email($email)) {
            $isUserFound = $this->db->query("SELECT * FROM `fc_users` WHERE `email` like '" . $email . "' AND `password` ='" . $pwd . "' ORDER BY `id` DESC");
    ...
    function send_password_to_mail()
    {
        ...
        if (valid_email($email)) {
            ...
            if ($isUserFound->num_rows() == 0) {
                ...
                echo 'Success::Email Send To your Email Account ';
            }
        }
    }
