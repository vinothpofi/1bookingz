...
class Commission extends MY_Controller
{
    ...
    public function display_commission_tracking_lists()
    {
        ...
        if ($this->checkLogin('A') == '') {
           ...
           $this->data['dateto'] = $this->input->post('dateto');
            ...
            $this->data['heading'] = 'Rental Commission Tracking';
            $this->load->view('admin/commission/display_tracking_lists', $this->data);
        }
    }
    ...
    public function display_commission_tracking_lists_datatables()
    {
        $requestData= $_REQUEST;
            ...
            if($rep_code!=''){
                //'status'=>'Active'
                $sellerDetails = $this->commission_model->get_all_details(USERS,array('group'=>'Seller','rep_code'=>$rep_code),array(array('field'=>'id','type'=>"DESC")));
            }else{
                //'status'=>'Active'
                $sellerDetails = $this->commission_model->get_all_details(USERS,array('group'=>'Seller'),array(array('field'=>'id','type'=>"DESC")));
            }
...
// $rep_code = 20;
             if($rep_code!=''){
                 //."' AND `status` = 'Active'";
                $condition = "`group` = 'Seller' AND `rep_code` = '".$rep_code." ";
            ...
            $this->db->where(array('status'=>'Active','host_status'=>0));
...
//  print_r($sellerDetails->result());exit;
            foreach ($sellerDetails->result() as $seller) {
                ...
                if($this->input->post('datefrom') != '' && $this->input->post('dateto') != '')
                {
                   ...
                   // echo $to_date;exit;
                    $rental_booking_details[$sellerEmail] = $this->commission_model->get_all_commission_tracking_with_date($sellerEmail,$from_date,$to_date);
                        ...
                        if ($checkinBeforeDay <= $current_date) {
                           ...
                           //$cancel_amountBf = $rentals['payable_amount'] - $rentals['paid_cancel_amount'];
                            ...
                            if ($rentals['cancelled'] == 'Yes') {
                                 if ($rentals['user_currencycode'] != $this->data['admin_currency_code']) {
                                     $cancel_amountBf1 = currency_conversion($rentals['user_currencycode'], $this->data['admin_currency_code'], $cancel_amountBf,$currencyCronId);
                                 }
                                 else
                                 {
                                     $cancel_amountBf1 = $cancel_amountBf;
                                 }
                                $this->data['trackingDetails'][$sellerEmail]['cancel_percentage'] += $cancel_amountBf1;
                            } else {
           ...
           foreach ($this->data['trackingDetails'] as $key => $value){
            $nestedData=array(); 
               ...
               if ($details['renter_id'] != '') {
                    $email_data_is = '<td class=" ">"'.$key.'"&nbsp;&nbsp;<br><a style="color: red;" href="'.base_url().'admin/bookingpayment/display_receivable/'.$renter_dat_is.'">View</a></td>';
                }else{
               ...
               $from_date_res='';
                 ...
                 if($from_date_res == '') {
                    $from_date_res_data='<a title="Click to view details" class="tip_top" href="'.base_url().'admin/commission/display_product_list/'.$renter_dat_is.'"><span class="badge_style">Properties&nbsp;</span></a>';
                  } else{
                        $from_date_res_data = '----';
                  }

               $balanceAmountIs = $details['payable_amount'] - $value['paid'];
               $attr = array('onsubmit' => 'return checkHostPaypalEmail('.$i.')');
               $paypal_form = form_open('admin/commission/paypal_payment',$attr);
               $paypal_amount = number_format($balanceAmountIs, 2);
               $paypal_amount_form = form_input([
                                                'type'     => 'hidden',
                                                'value'    => $paypal_amount,
                                                'name'     => 'amount_from_db'
                                                 ]);
                                        if ($this->data['admin_currency_code'] != 'USD') 
                                        {
                    ...
                    $amount_to_pay = form_input([
                                            ...
                                            $bookingNo= form_input([
                                                'type'     => 'hidden',
                                                'value'    => $details['booking_noPay'],
                                                'name'     => 'bookingNo'
                                            ]);

                                            if ($this->data['paypalData'][$key] != '')
                                            {
                                                $paypalemil = $this->data['paypalData'][$key];
                                            }
                                            else
                                            {
                                                $paypalemil = no;
                                            ...
                                            $online_paypal =  form_input([
                                                ...
                                                'style'    => 'border: #cb9b0c 1px solid;font-size: 10px;',
               ...
               if($from_date_res == '') {
                  $attr = array('onsubmit' => 'return checkHostPaypalEmail('.$i.')');
                  if (number_format($balanceAmountIs, 2) > 0.00) {
                       $pay_button = ' <span class="action_link"><a class="p_approve tipLeft" href="admin/commission/add_pay_form/'.$details_id.'" title="Offline Payment Mode" style="margin-bottom:2px;">Offline</a></span><span class="action_link">'.$paypal_form.$paypal_amount_form.$amount_to_pay.$hostEmail.$checkinDate.$bookingNo.$hostPayPalEmail.$online_paypal.form_close().'</span><span></span>';
                                             }
                                             else{
                                           $pay_button = 'Paid';
                                        } 
                ...
                $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['coupon_discount'], 2);
                $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['guest_fee'], 2);
                $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['cancel_percentage'], 2);
                $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['booking_walletUse'], 2);
                $nestedData[] = $admin_currency_symbol . ' ' . number_format((($details['guest_fee'] + $details['host_fee']) - $details['booking_walletUse']-$details['coupon_discount']), 2);
                ...
                $nestedData[] = $admin_currency_symbol . ' ' . number_format($balanceAmountIs, 2);
    ...
    public function display_wallet_payments_list()
    {
        if ($this->checkLogin('A') == '') {
            ...
            foreach ($sellerDetails->result() as $seller) {
                $userId = $seller->id;
                $wallet_applyCount = "select sum(py.wallet_Amount) as usedWallet,py.currency_code,re.currencyPerUnitSeller,re.user_currencycode,re.currency_cron_id from " . PAYMENT . " as py ," . RENTALENQUIRY . " as re where py.user_id='" . $userId . "' and py.is_wallet_used='Yes' and py.wallet_Amount!='0.00' and py.EnquiryId=re.id";
                ...
                if ($wallet_usage->usedWallet != null) {
                    ...
                    $currency_code = $wallet_usage->user_currencycode;
                    if ($currency_code != $this->data['admin_currency_code']) {
                        $usedWallet = currency_conversion($currency_code, $this->data['admin_currency_code'], $usedWallet,$wallet_usage->currency_cron_id);
                        //customised_currency_conversion($currencyPerUnitSeller, $usedWallet);
                ...
                if (count($rental_booking_details[$userEmail]) != 0) {
                    foreach ($rental_booking_details[$userEmail] as $rentals) {
                        if ($rentals['user_currencycode'] != $this->data['admin_currency_code']) {
                            $rentals['guest_fee'] = currency_conversion($rentals['user_currencycode'], $this->data['admin_currency_code'], $rentals['guest_fee'],$rentals['currency_cron_id']);
                            //customised_currency_conversion($currencyPerUnitSeller, $rentals['guest_fee']);
                        } else {
                            $rentals['guest_fee'] = $rentals['guest_fee'];
                        }
                        //echo $rentals['booking_no'].'|'.$userEmail.'|'.$rentals['guest_fee'].'<br>';
    ...
    public function display_wallet_payments_list_datatables()
    {
        $this->data['heading'] = 'User Wallet Lists';
            ...
            if ($rep_code != '') {  
                $condition = "`status` = 'Active' AND `email` IS NOT NULL AND `rep_code` = ".$rep_code."";
            }else{
            ...
            $this->data['tot_usedWallet'] = $tot_usedWallet;
                            ...
                            $data=array();
                            $i = 1;
                            foreach ($this->data['walletData'] as $details)
                            {
                              $nestedData=array();
                              $nestedData[] = $i;
                              $nestedData[] = $details['name'];
                              $nestedData[] = $details['email'];
                              $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['totalReferalAmount'], 2);
                              $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['usedWallet'], 2);
                              $nestedData[] = $admin_currency_symbol . ' ' . number_format($details['totalReferalAmount'] - $details['usedWallet'], 2);
                              $data[] = $nestedData;
                                $i++;
    ...
    /**/
    public function online_pay_by_stripe()
    {
        $this->data['heading'] = 'Add Stripe payment';
        $seller_id = $this->uri->segment(4);
        echo $seller_id;    
        $this->load->view('admin/commission/add_stripe_payment', $this->data);
    } 

    public function HostPaymentCreditStripe()
        {
            //echo '<pre>'; print_r($_POST);die;
            $condition = array('id' => $this->checkLogin('U'));
            $userDetails = $this->commission_model->get_all_details(USERS, $condition);
            ...
            $product = $this->commission_model->get_all_details(PRODUCT, array('id' => $product_id));
            $seller = $this->commission_model->get_all_details(USERS, array('id' => $product->row()->user_id));
            ...
            $secret_key = $this->input->post('stripe_key');//$StripeVals['secret_key'];
            $publishable_key = $this->input->post('stripe_publish_key');//$StripeVals['publishable_key'];
            ...
            try {
               // $customer = Stripe_Customer::create(array("card" => $token, "description" => "Product Purhcase for " . $this->config->item('email_title'), "email" => $this->input->post('email')));
                ...
                if ($this->session->userdata('randomNo') != '') {
                    ...
                    $this->commission_model->ExecuteQuery($delete, 'delete');
                ...
                $this->commission_model->simple_insert(PAYMENT, $paymentArr);
    ...
    function paypal_commission_success()
    {
       ...
       /* $sellerEmail = $this->commission_model->get_commission_tracking_list($custom_values[0]);
        ...
        $Get_bk_numbers = $this->commission_model->get_unpaid_commission_tracking($custom_values[0]); 
        foreach ($Get_bk_numbers as $rentals_up) 
        {
            $bk_numbers[]=$rentals_up['booking_no'];
        }*/
