...
class Membership extends MY_Controller {
	...
	public function display_membership_dashboard(){
		...
		if ($this->checkLogin('A') == ''){
			...
			$this->data['MembershipCount'] = $this->membership_model->get_fancybox_count_details();
			$this->data['MembershipList'] = $this->membership_model->get_fancybox_View_details();			
	...
	public function membership_list(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['MembershipList'] = $this->membership_model->get_fancybox_list_details();
	...
	public function insertEditProduct(){
		if ($this->checkLogin('A') == ''){
			...
			if ($valid_date == ''){
				$this->setErrorMessage('error','Month is required');
				echo "<script>window.history.go(-1)</script>";exit();
			}
