...
class Listattribute extends MY_Controller {
	...
	public function add_listattribute_form(){
		if ($this->checkLogin('A') == ''){
			redirect('admin');
		}else {
	...
	public function insertAttributelist(){
		if ($this->checkLogin('A') == ''){
			...
			if($attribute_name ==''){
				...
				redirect('admin/listattribute/add_attribute_listform/');
			}
			...
			$this->setErrorMessage('success','List added successfully!!');
			redirect('admin/listattribute/display_attribute_listspace');
		}
	}
	...
	public function delete_list_value(){
		...
		if ($this->checkLogin('A') == ''){
			...
			$r_list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES,array('status'=> 'Active','listspace_id' => '10'));
			$p_list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES,array('status'=> 'Active','listspace_id' => '9'));
			$list_value_id = $this->uri->segment(4,0);
			$is_room_type = $this->attribute_model->get_all_details(LISTSPACE_VALUES,array('status'=> 'Active','listspace_id' => '10','id' => $list_value_id));
			if($is_room_type->num_rows() == 1){
				if($r_list_count->num_rows() == 1)
				{
					$this->setErrorMessage('error','At Least One Room type Should Be Active');
					redirect('admin/listattribute/display_listspace_values');
				}
			}else{
			if($p_list_count->num_rows() == 1){
				$this->setErrorMessage('error','At Least Property type One Should Be Active');
            	redirect('admin/listattribute/display_listspace_values');
			}
    ...
    public function change_list_value_status_global(){
        ...
        if(count($this->input->post('checkbox_id')) > 0 &&  $this->input->post('statusMode') != ''){
            //echo '<pre>';
            //print_r($this->input->post('checkbox_id'));
            $ids=$this->input->post('checkbox_id');
            $ids = array_diff($ids, array('on'));
            $status='Active';
            $mode=strtolower($this->input->post('statusMode'));

            if($mode == 'delete' || $mode=='inactive') {
                $res = $this->attribute_model->get_listspace_total_count($ids,$status);
                //print_r($res->result());
                $str='';
                $err=array();
                $all_res = $res->result();
                foreach ($all_res as $val) {
                    if ($val->child_ids != '') {
                        $ch_arr = explode( ',',$val->child_ids);
                        $arr_dif=array_diff($ch_arr,$ids);
                        //print_r($arr_dif);
                        if(empty($arr_dif)){
                            $err[]=1;
                            //$this->setErrorMessage('error', 'At Least One Property Type and Room Type Should Be Active');
                        }
                    }
                }
            }
            ...
            if(!empty($err)){
                $this->setErrorMessage('error', 'Atleast one Property Type and Room Type values should be Active');
                redirect('admin/listattribute/display_listspace_values');
            }else{
                $this->attribute_model->activeInactiveCommon(LISTSPACE_VALUES,'id');
                if (strtolower($this->input->post('statusMode')) == 'delete'){
                    $this->setErrorMessage('success','Model deleted successfully');
                }else {
                    $this->setErrorMessage('success','Model status changed successfully');
                }
                redirect('admin/listattribute/display_listspace_values');
            }
        ...
        // print_r($_POST);exit();
        if (count($this->input->post('checkbox_id')) > 0 && $this->input->post('statusMode') != '') {
            //print_r($this->input->post('checkbox_id'));exit();
            ...
            // $this->db->where_in(array('id'=> $data,'status' => 'Active'));
            // $demo = $this->db->get(LISTSPACE_VALUES);
            $r_list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES, array('listspace_id' => '10'));
            $p_list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES, array('listspace_id' => '9'));

            $all_rows = $r_list_count->num_rows() + $p_list_count->num_rows();
            if ($all_rows == count($data)) {
                $this->setErrorMessage('error', 'You Cant Modify all at the time');
                redirect('admin/listattribute/display_listspace_values');
            }
            if (strtolower($_POST['statusMode']) == 'delete') {
                for ($i = 0; $i < count($data); $i++) {
                    $condition = array('status' => 'Active');
                    $list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES, $condition);
                    $list_count_is = $list_count->num_rows();
                    $r_list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES, array('status' => 'Active', 'listspace_id' => '10'));
                    $p_list_count = $this->attribute_model->get_all_details(LISTSPACE_VALUES, array('status' => 'Active', 'listspace_id' => '9'));
                    if ($r_list_count->num_rows() == 1 || $p_list_count->num_rows() == 1) {
                        $this->setErrorMessage('error', 'At Least One Property Type and Room Type Should Be Active');
                        redirect('admin/listattribute/display_listspace_values');
                    }
                    $this->attribute_model->commonDelete(LISTSPACE_VALUES, array('id' => $data[$i]));
                }
                $this->setErrorMessage('success', 'deleted successfully');
            } else {
                for ($i = 0; $i < count($data); $i++) {

                    $status = ($_POST['statusMode'] == 'Inactive') ? 'Inactive' : 'Active';
                    $other = ($_POST['statusMode'] == 'Inactive') ? 'No' : 'Yes';
                    ...
                    $condition = array('id' => $data[$i]);
                    $this->attribute_model->update_details(LISTSPACE_VALUES, $newdata, $condition);
	...
	public function insertEditListSpaceValue(){
		...
		else
		{
			...
			//$list_description_ar = $this->input->post('list_description_ar');
			$seourl = url_title($list_value,'',TRUE);
			...
			$dataArr = array(
				'listspace_id'	=>	$list_id,
				'list_value'=>	$list_value,
				//'list_value_ar'=>	$list_value_ar,
				'list_description'=>	$list_description,

				'other'=>	'Yes',
				//'list_description_ar'=>	$list_description_ar,
				'list_value_seourl'=> $seourl
			);		

			 foreach(language_dynamic_enable("list_description","") as $dynlang) {
               $dataArr=array_merge($dataArr,array($dynlang[1] => $this->input->post($dynlang[1])));
            }	
		    ...
		    $this->load->library('upload', $config);
				...
				if($lvID!='')
				{
					...
					redirect('admin/listattribute/edit_listSpace_value_form/'.$lvID);
				}
				else
				{
					$this->setErrorMessage('error','File Should be JPEG,JPG,PNG and below 100*100 px');
					redirect('admin/listattribute/add_listspace_value_form');
				}					
				}
			}
	...
	public function insertAttribute_other(){
		if ($this->checkLogin('A') == ''){
			...
			if($attribute_name ==''){
				$this->setErrorMessage('error','Please enter list name');
				redirect('admin/listattribute/add_lang_value');
			}
	...
	public function insertEditListSpaceValue_other(){
		if ($this->checkLogin('A') == ''){
			...
			if($attribute_name ==''){
				$this->setErrorMessage('error','Please enter list name');
				redirect('admin/listattribute/add_lang_value_space');
			}
