	...
	this.process = function(from, to, id){
		...
		for (var i = 0; i < month.length; i++) {
			...
			for (var j = 0; j < month[i].length; j++) {
				...
				ny = currentDate.getFullYear();
				nm = currentDate.getMonth()+1;
				nD = currentDate.getDate();
				//html+= ny+'-'+nm+'-'+nD;(if you make it visible current date will show multiple time)
				//html+=month[i][j].date;
				if(month[i][j].date < (ny+'-'+nm+'-'+nD)){
					html += '<td class="">';
				}
				else{
					html += '<td>';
