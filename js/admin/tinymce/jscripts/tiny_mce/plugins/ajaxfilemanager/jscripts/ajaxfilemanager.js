...
function initAfterListingLoaded()
{
	...
	if(view != '')
	{
		...
		switch(view)
		{
			...
			case 'image':
				//enableContextMenu('dl.imageListing, dl.imageListing dt, dl.imageListing dd, dl.imageListing a');
				enableContextMenu('dl.imageListing');
				for(i in files)
				{
...
function deleteDocuments()
{
	...
	{
		case 'image':
			var selectedDoc = $('#rightCol dl.imageListing input[@type=checkbox][@checked]');
			break;
		case 'detail':
					...
					}else
					{
						...
						switch(getView())
						{
							case 'image':
							 	...
							 	$('#thumbImg' + num).attr('src', appendQueryString(getUrl('image'), 'path=' + files[num].path, ['path']));
...
function checkAll(checkbox)
{
	...
	if($(checkbox).attr('class') == "check_all")
	{
		...
		{
			case 'image':
				$('#rightCol dl.imageListing input[@type=checkbox]').each(function(i){
		...
		switch(getView())
		{
			...
			case 'image':
				...
				{//this is file item
					...
					{
						case 'filePicture':
							$('<a id="thumbUrl' + num + '" rel="thumbPhotos" href="' + files[num].path + '"><img src="' + appendQueryString(getUrl('image', false, false), 'path=' + files[num].path, ['path']) + '" id="thumbImg' +  num + '"></a>').appendTo('#dt' + num);
							break;
						case 'fileFlash':
							break;
						case 'fileVideo':
