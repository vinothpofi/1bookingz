...
{ switch(view)
{ case 'image':
enableContextMenu('dl.imageListing'); for(i in files)
...
{ switch(getView())
{ case 'image': $('#dl' + num ).remove(); break; case 'detail':
default:
$('#row' + num).remove();}
files[num] = null;}
} );}
...
switch(getView())
{ case 'image':
var selectedDoc = $('#rightCol dl.imageListing input[@type=checkbox][@checked]'); break; case 'detail':
default:
...
switch(getView())
{ case 'image':
$('#thumbUrl' + num).attr('href', files[num].path); $('#thumbImg' + num).attr('src', appendQueryString(getUrl('image'), 'path=' + files[num].path, ['path'])); $('#cb' + num).val(files[num].path); $('#a' + num).attr('href', files[num].path).text(files[num].name); break; case 'detail':
default:
...
{ case 'image':
$('#rightCol dl.imageListing input[@type=checkbox]').each(function(i){ $(this).attr("checked", 'checked');}) ; break; case 'detail':
default:
...
{ case 'image':
$('#rightCol dl.imageListing input[@type=checkbox]').each(function(i){ $(this).removeAttr("checked");}) ; break; case 'detail':
default:
...
switch(getView())
{ case 'image':
$( '<dl class="imageListing" id="dl' + num + '" ><dt id="dt' + num + '" class="' + files[num].cssClass + '"></dt><dd id="dd' + num + '" class="imageListing_info"><span id="flag' + num + '" class="' + files[num].flag + '">&nbsp;</span><input id="cb' + num + '" type="checkbox"  class="radio" ' + strDisabled +' name="check[]" class="input" value="' + files[num].path + '" /><a href="' + files[num].path + '" title="' + files[num].name + '" id="a' + num + '">' + (typeof(files[num].short_name) != 'undefined'?files[num].short_name:files[num].name) + '</a></dd></dl>').appendTo('#content'); if(files[num].type== 'folder')
{ enableFolderBrowsable(num);}else
{ switch(files[num].cssClass)
{ case 'filePicture':
$('<a id="thumbUrl' + num + '" rel="thumbPhotos" href="' + files[num].path + '"><img src="' + appendQueryString(getUrl('image', false, false), 'path=' + files[num].path, ['path']) + '" id="thumbImg' + num + '"></a>').appendTo('#dt' + num); break; case 'fileFlash':
break; case 'fileVideo':
break; case 'fileMusic':
break; default:
}
