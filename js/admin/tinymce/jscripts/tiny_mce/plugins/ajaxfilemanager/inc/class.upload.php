...
class Upload
{
	...
	/**
	 * resize the image and return the image image  details array("width"=>, "height"=>, "name")
	 *
	 * @param string $fileName
	 * @param int $new_x the image width
	 * @param int $new_y the image height
	 * @return unknown
	 */
	...
	/**
	 * save the image file and destroy the opened image
	 *
	...
	function _imageSave($newImageHandler, $fileName, $quality = 90)
	{
		...
		{
			imagedestroy($newImageHandler);
			array_push($this->errors, "Unable to save the image file.");
			return false;
		}
	...
	/**
	 * caculate the image details from the original image file
	 *
	 ...
	 */
	function getThumbInfo($originalImageName, $originaleImageWidth, $originalImageHeight, $imageSuffix, $imageWidth, $imageHeight)
	{
		...
		if(!empty($originalImageName) && !empty($originaleImageWidth) && !empty($originalImageHeight))
		{
			...
			//begin to get the image image name
			$fileExtension = '';
			$fileBaseName = '';
			if(is_int($dotIndex))
			{
			...
			$outputs['name'] = $fileBaseName . $imageSuffix . $fileExtension;
			//start to get the image width & height
			if($imageWidth < 1 && $imageHeight < 1)
			{
				$imageWidth =$originaleImageWidth;
				$imageHeight = $originalImageHeight;
			}elseif($imageWidth < 1)
			{
				$imageWidth = floor($imageHeight / $originalImageHeight * $originaleImageWidth);

			}elseif($imageHeight < 1)
			{
				$imageHeight = floor($imageWidth / $originaleImageWidth * $originalImageHeight);
			}else
			{
				$scale = min($imageWidth/$originaleImageWidth, $imageHeight/$originalImageHeight);
				$imageWidth = floor($scale*$originaleImageWidth);
				$imageHeight = floor($scale*$originalImageHeight);
			}
			$outputs['width'] = $imageWidth;
			$outputs['height'] = $imageHeight;
		}
		return $outputs;
	 ...
	 * @param string $originalImageName
	 * @param string $arrayimageSuffix
	 */
	function deleteFileAndThumbs($dirPath, $originalImageName, $arrayimageSuffix)
	{
		//ensure the directory path ending with /
