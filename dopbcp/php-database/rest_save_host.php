	...
	if (isset($_POST['dopbcp_calendar_id'])){ 
		...
		foreach($dat_arr as $key=>$val)
		{
			foreach($val->selectedTiming as $sel_time)
			{
				...
				if($num_rows == 0)
				{
					$query = "INSERT INTO  fc_rest_bookings_dates (id_item, the_date, id_state, rest_type, rest_id, hall_id, price, checked_in, checked_out, tot_checked_in, tot_checked_out, tot_time, added_by) VALUES ('1','".$key."', '".$id_stateIs."', '6', '".$propId."', '".$hall_id."', '".$val->price."', '".$checked_in."', '".$checked_out."', '".$tot_checked_in."', '".$tot_checked_out."', '".$tot_time."','host')";
					echo $query;
					mysqli_query($conn, $query);
				}
				else
